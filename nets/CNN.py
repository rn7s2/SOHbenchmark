...
class ResBlock(nn.Module):
    def __init__(self, input_channel, output_channel, stride):
        ...
        if output_channel != input_channel:
            self.skip_connection = nn.Sequential(
                nn.Conv1d(input_channel, output_channel, kernel_size=1, stride=stride), nn.BatchNorm1d(output_channel)
            )
...
class CNN(nn.Module):
    ...
    def __init__(self):
        ...
        self.predictor = nn.Sequential(nn.Linear(128 * 8, 128), nn.ReLU(), nn.Linear(128, 1))
    ...
    def forward(self, x):
        ...
        out = self.layer4(out)
        out = self.layer5(out)
        pred = self.predictor(out.reshape(out.size(0), -1))
        return pred
